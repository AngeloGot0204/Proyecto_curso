...
- [x] The `adjuntos-reporte` scenario "Attachment queued via existing FormData submit path" reads literally as *sync through `fetch(form.action, …)`*; D2 keeps that mechanism but targets the attachment endpoint, since bundling would violate the same spec's isolation requirement. **Resolved**: confirmed the design's reading; the spec scenario was amended before `sdd-verify` to describe the dedicated-endpoint mechanism explicitly.
- [x] `SECCION_DE_ADJUNTOS` needs the real S-08 `seccion.id` from the production definition YAML — the sample definitions in `media/` are empty (`secciones: []`). **Resolved (2026-08-30)**: inspected the real deployed `instalacion-resinas` template (`REPORTE_DE_INSTALACION_DE_RESINAS...xlsx`, sheet `JME.PC-0001.F1`) — its "CROQUIS DE ZONA DE TRABAJO:" block (cell B53) lives inside the RESULTADOS block, matching the wizard's `resultados` section id exactly. `SECCION_DE_ADJUNTOS`, `paso.html`'s two `{% if seccion.id == ... %}` gates, and the `seccion_s08_id` test fixture were all updated from the placeholder `"s-08-croquis-evidencia"` to `"resultados"` and re-verified live in the browser (Phase 6).
- [ ] Anchor-slot box defaults (320×240 px, aspect-preserving fit) are an assumption; confirm against the real `.xlsx` layout once the template declares its four slots. **Still open** — user deferred configuring the four anchor-slot cells (rows 54-60, below "CROQUIS DE ZONA DE TRABAJO" at B53, above signatures at row 61) to be done later via the Django admin, once `DefinicionDeTipo.estructura["adjuntos"]` gets its slot list. Until then, `_incrustar_adjuntos` correctly no-ops (no declared slots = nothing embedded), a documented, non-crashing default.
- [x] Pinned CDN versions/SRI for `heic2any` and `browser-image-compression` — the existing Dexie tag uses `crossorigin="anonymous"` with no `integrity`. **Resolved**: matched that precedent as-is (no `integrity` attribute added), consistent with D3's stated no-SRI convention across all three CDN scripts in this project.
