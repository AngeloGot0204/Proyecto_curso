...
|---|---|---|---|---|
| 1 | Modelo base y autenticación | **✓ DONE** — `Usuario` con rol admin/usuario, login por sesión Django, admin crea cuentas (ADR-0005) — "admin crea cuentas" se resuelve con pantalla propia dentro de la app (S-13, admin de usuarios: buscador, crear, editar rol/organización, resetear contraseña, suspender), consistente con el resto de módulos admin (S-14), en vez de solo Django admin. Spec: `usuarios-y-autenticacion` | — | — |
| 2 | Despliegue e infraestructura | **✓ DONE** — Vercel + Neon (Postgres) + Vercel Blob, HTTPS automático (ADR-0009). Spec: `despliegue-e-infraestructura` | #1 | — |
| 3 | Motor de definición de tipo de reporte | **✓ DONE** — `TipoDeReporte` + `DefinicionDeTipo`: carga declarativa, validación al activar (secciones/campos/mapeo celda), rechazo de definición inválida (ADR-0003, ADR-0008). Spec: `motor-definicion-tipo-reporte` | #1 | — |
| 4 | Generador de Excel desde plantilla | **✓ DONE** — Servicio que escribe `ValorDeReporte` sobre plantilla `.xlsx` original vía openpyxl, preserva formato/celdas combinadas/logo (ADR-0002) | #3 | — |
| 5 | Wizard de captura server-rendered | **✓ DONE** — Formulario multi-paso generado en servidor leyendo `DefinicionDeTipo`, sin código por tipo de reporte, sin offline todavía (ADR-0001). El `Reporte` lleva `eliminado_en` para borrado lógico: todo acceso lo excluye, y las filas relacionadas sobreviven para auditoría. Specs: `wizard-captura`, `reportes-modelo` | #3 | — |
| 6 | Validación de datos del formulario | **✓ DONE** — Campos obligatorios, formato hora/fecha, hora término > inicio, advertencia "No cumple" sin bloqueo, pantalla S-09 con enlaces a campo | #5 | — |
| 7 | Cierre manual (visto bueno) y generación del documento | **✓ DONE** — `VistoBueno` (solo el creador cierra), `Generacion`, endpoint que dispara el generador (#4), fallo limpio si plantilla/mapeo inválido (ADR-0006, ADR-0008) | #4, #6 | — |
| 8 | Colaboración por invitación y edición abierta | **✓ DONE** — `ParticipacionEnReporte`, invitar usuario, acceso restringido a invitados, `CambioDeValor` con auditoría FIFO 30 (ADR-0006). La pantalla de revisión (S-09) es dueña del cierre; Participantes solo lista invitados e historial | #7 | — |
| 9 | Capa offline (IndexedDB + service worker) | **✓ DONE** — Persistencia local de borrador, funciona en modo avión, navegación entre pasos sin pérdida de datos (ADR-0004). Barra de pantalla incluye chip de conexión en vivo (offline/en línea, vía `navigator.onLine` + listener) en las 16 pantallas con barra. Specs: `capa-offline`, `upload-queue` | #8 | — |
| 10 | ~~Sincronización y asignación de número de registro~~ **✓ DONE** | Cola de subida, ID local idempotente, secuencia de BD para número de registro, reintento manual, sesión expirada no descarta borrador (ADR-0004, ADR-0005). Incluye pantalla agregada de sincronización (S-15): lista única de todos los pasos pendientes/fallidos entre reportes (tipo, fecha, paso), con reintento por fila, sin tener que entrar reporte por reporte — Specs: `reporte-idempotent-creation`, `upload-queue`, `sincronizacion-pendientes` | #9 | — |
